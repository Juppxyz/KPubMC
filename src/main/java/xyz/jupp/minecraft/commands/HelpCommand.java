    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player) {
            Bukkit.getScheduler().runTaskAsynchronously(Main.getInstance(), () -> {
                ...
                player.sendMessage("§8§l» §a/einladungen §8- §fÄndert, ob du offen für Team-Anfragen bist.");
                player.sendMessage("§8§l» §a/slimechunk §8- §fÜberprüft ob du in einem SlimeChunk bist.");
                player.sendMessage("§8§l» §a/geld §8- §fRuft dein Konto auf.");
                player.sendMessage("§8§l» §a/ranking §8- §fZeigt das Team Ranking an.");
                player.sendMessage("§8§l» §a/head <Spielername> §8- §fKauft den Kopf (Item) des Spielers.");
